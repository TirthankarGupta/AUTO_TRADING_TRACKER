...
# Single-file robust Streamlit trading dashboard (mock-data friendly)
...
# small CSS for header/card
st.markdown(
    """
    <style>
      .header-pill { display:inline-block; width:36px; height:36px; border-radius:8px; background:#FF7A2D; margin-right:10px; }
      .center-card { border-radius:8px; box-shadow: 0 4px 10px rgba(2,6,23,0.06); padding:4px 8px; background:white; }
    </style>
    """,
...
with right_col:
    ...
    st.markdown("---")
    st.markdown("<small>Note: toggle 'Simulate data' to run without backend.</small>", unsafe_allow_html=True)

# Helpers
def simulate_quotes(symbol: str, count: int, tf: str = "1m"):
    now = pd.Timestamp.now().floor("T")
    freq = {"1m":"T", "5m":"5T", "15m":"15T"}.get(tf, "T")
...
def get_data_safe(symbol, count, simulate_flag, tf):
    if simulate_flag:
        return simulate_quotes(symbol, count, tf), None
    try:
        df = fetch_quotes_from_api(symbol, count)
        if df.empty:
            return simulate_quotes(symbol, count, tf), "Backend returned empty data, using simulated data."
        return df, None
    except Exception as e:
        return simulate_quotes(symbol, count, tf), f"Quote fetch error: {e}"

# Main
if refresh or True:
    df, err = get_data_safe(symbol, int(count), simulate, timeframe)
    if err:
        message_placeholder.error(err)
    else:
        message_placeholder.empty()

    # Plotting: try plotly, else fallback
    try:
