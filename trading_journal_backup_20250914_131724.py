"""
trading_journal.py  (FINAL UI PATCH)
- Fixes: header pinned to very top (no gap), status/log inside header, black silhouettes on softened candles background,
  CONTROL font larger, sidebar deep navy, white buttons with black text, zebra journal with No. column.
- Defensive: falls back to mock data if real providers missing; never prints secrets.
Author: Assistant (final patch)
Date: 2025-09-14 (ui-final)
"""
...
try:
    ...
    raise RuntimeError("Streamlit required. Install with: pip install streamlit") from e

try:
    import plotly.graph_objects as go
    from plotly.subplots import make_subplots
except Exception as e:
    raise RuntimeError("Plotly required. Install with: pip install plotly") from e

# --------------------------
# Embedded header SVG (base64)
# - soft candle background (muted), black bull & bear silhouettes
# --------------------------
SVG = r'''
<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="160" viewBox="0 0 1600 160">
  ...
  <!-- Soft, shallow candlestick background (muted, slightly offset) -->
...
# --------------------------
# Status retrieval & sanitization (safe)
# --------------------------
...
def get_app_status():
    # 1) try smartapi_session_manager
    try:
...
# --------------------------
# Data provider fallbacks (unchanged)
# --------------------------
def try_get_candles():
    try:
        import trading_data
...
# --------------------------
# Streamlit page config and CSS adjustments
# - header fixed at top with height HEADER_H; main container is padded accordingly so nothing overlaps.
# --------------------------
st.set_page_config(page_title="Trading - Momentum Surge Scalping", layout="wide", initial_sidebar_state="expanded")

HEADER_H = 160  # px - header fixed height
...
/* Main header card inside fixed area */
.main-header-card {{
...
/* Make sure app content begins AFTER header */
.reportview-container .main .block-container {{
    padding-top: {HEADER_H + 12}px; /* push content below the fixed header */
    padding-left: 1rem;
    padding-right: 1rem;
}}
...
.header-title h1 {{ margin:0; font-size:20px; color:{WHITE}; }}
.header-title p {{ margin:2px 0 0 0; font-size:11px; color:#d1e7ff; }}

/* Status badge inside header (right aligned) */
.status-badge {{
  margin-left:auto;
  background: #ffffff;
  color: #041723;
...
/* Sidebar styling: deep navy panel, inputs white bg and black text */
.stSidebar {{ background: {DEEP_NAVY} !important; padding: 12px !important; }}
.control-header {{
    color: {WHITE};
    ...
    text-align:center;
    font-size:18px;  /* increased font for CONTROL */
    letter-spacing:0.6px;
    border: 1px solid rgba(255,255,255,0.04);
}}
/* Buttons white with black text in sidebar */
...
/* Checkbox labels forced white in sidebar for contrast */
.stSidebar label, .stSidebar .stMarkdown, .stSidebar .stText {{ color: #ffffff !important; }}
...
# --------------------------
# Render fixed header area (image + title + status badge) via HTML inside a fixed container.
# We keep the outer wrapper (.app-header) fixed and child .main-header-card visually centered.
# --------------------------
...
<div class="app-header" role="banner">
  <div class="main-header-card">
    <div style="width:160px; display:flex; align-items:center; padding-right:8px;">
      <img src="{HEADER_IMAGE_B64}" alt="header" style="height:120px; width:auto; display:block; border-radius:4px;"/>
    </div>
    <div class="header-title">
      <h1>Trading - Momentum Surge Scalping</h1>
      <p>Candles • EMA9/EMA21 • VWAP • RSI • Only BUY CE/PE</p>
    </div>
    <div class="status-badge" title="{status_msg}">{status_msg}</div>
  </div>
</div>
"""
...
# --------------------------
# PnL row (keeps same behavior) - will appear below fixed header due to padding above.
# --------------------------
