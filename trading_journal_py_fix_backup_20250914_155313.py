"""
trading_journal.py - Final tweak: ensure selectbox (Candle frequency) selected text and Actions button text are BLACK.
Overwrite existing file with this one and run: streamlit run trading_journal.py
"""
import base64
import os
import math
...
# -------------------------
# Logo embedding (prefer logo.png)
# -------------------------
...
# -------------------------
# Streamlit config + CSS (tweak to force select/button text black)
# -------------------------
...
/* remove Streamlit top chrome and tighten container */
...
/* Sidebar width & placement */
.stSidebar {{ background: {DEEP_NAVY} !important; padding-top: 12px !important; width: {SIDEBAR_WIDTH}px; }}

/* CONTROL header: larger, pushed up */
.control-header {{
  color:#ffffff;
  font-weight:900;
  font-size:22px;
...
/* HERO: positioned to the right of sidebar, with a comfortable gap */
.hero {{
  width: calc(100% - {SIDEBAR_WIDTH + 40}px);
...
/* Logo: remove white framing so it blends with hero bg */
...
/* ensure placeholder text readable */
...
/* Strong black labels for Candle frequency and Actions */
.sidebar-label-black {{ color:#000000 !important; font-weight:700 !important; margin-bottom:6px; display:block; }}

/* --- NEW: force selected text inside Streamlit selectbox and button labels to be black --- */
/* Attempts to cover different Streamlit versions / DOM shapes */
.stSidebar .stSelectbox, .stSidebar .css-1t5f0fr, .stSidebar select, .stSidebar div[role="combobox"] {{
...
/* extra safe rule for nested spans */
.stSidebar span, .stSidebar .css-1v0mbdj span {{
  color: #000000 !important;
}
</style>
"""
...
# -------------------------
# Sidebar controls (Candle frequency & Actions use black labels)
# -------------------------
with st.sidebar:
