"""
trading_journal.py  (HEADER LEFT FIX - final replacement)
- Fix: HTML escaping removed, status pill reliably rendered inside header (left of title).
- Header fixed to top; sidebar pushed down by header height.
- Sidebar deep navy; buttons white with black text; CONTROL font larger.
- Trading Journal: No. column + zebra stripes.
- Defensive: falls back to mock data if providers missing.
Author: Assistant
Date: 2025-09-14 (header-left final)
"""
...
# --------------------------
# Embedded header SVG (base64)
# --------------------------
SVG = r'''
<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="160" viewBox="0 0 1600 160">
  <defs>
    <linearGradient id="softbg" x1="0" x2="0" y1="0" y2="1">
      <stop offset="0%" stop-color="#072a3a" stop-opacity="0.98"/>
      <stop offset="100%" stop-color="#0b3546" stop-opacity="0.98"/>
    </linearGradient>
  </defs>
  <rect width="1600" height="160" fill="url(#softbg)"/>
  <g transform="translate(80,12)" opacity="0.12">
    <g fill="#66d283">
      <rect x="0" y="90" width="8" height="40" rx="2"/>
      <rect x="28" y="84" width="8" height="46" rx="2"/>
      <rect x="56" y="78" width="8" height="52" rx="2"/>
      <rect x="84" y="72" width="8" height="58" rx="2"/>
      <rect x="112" y="66" width="8" height="64" rx="2"/>
      <rect x="140" y="62" width="8" height="68" rx="2"/>
      <rect x="168" y="56" width="8" height="74" rx="2"/>
      <rect x="196" y="50" width="8" height="80" rx="2"/>
      <rect x="224" y="44" width="8" height="86" rx="2"/>
      <rect x="252" y="38" width="8" height="92" rx="2"/>
    </g>
  </g>
  <g transform="translate(320,14) scale(0.9)" fill="#000000">
  ...
  <g transform="translate(1080,18) scale(0.95)" fill="#000000">
  ...
  <text x="32" y="36" font-family="Helvetica, Arial, sans-serif" font-size="22" fill="#ffffff" font-weight="700">
    Trading - Momentum Surge Scalping
  </text>
  <text x="32" y="56" font-family="Helvetica, Arial, sans-serif" font-size="11" fill="#d1e7ff">
...
# --------------------------
# Status retrieval & sanitization
# --------------------------
def sanitize_status(text: str, max_len: int = 140) -> str:
    if not text:
        return ""
    s = str(text)
...
def get_app_status():
    try:
        ...
        if hasattr(ssm, "get_status"):
            try:
                return sanitize_status(ssm.get_status())
            except Exception:
                pass
    except Exception:
        pass
...
# --------------------------
# Streamlit page config and robust CSS (no HTML comments in injected HTML)
# --------------------------
st.set_page_config(page_title="Trading - Momentum Surge Scalping", layout="wide", initial_sidebar_state="expanded")

css = f"""
<style>
html, body, .stApp {{ height:100%; margin:0; padding:0; background:{CANVAS_BG}; }}
.app-header {{ position: fixed; top: 0; left: 0; right: 0; height: {HEADER_H}px; z-index: 9999; display:flex; align-items:center; justify-content:center; padding: 10px 0; }}
.main-header-card {{ width:calc(100% - 40px); background: linear-gradient(90deg, {DEEP_NAVY} 0%, #062033 100%); color:{WHITE}; padding:10px 16px; border-radius:6px; display:flex; align-items:center; gap:12px; box-shadow: 0 2px 6px rgba(0,0,0,0.08); }}
.main-header-card img {{ height:120px; width:auto; border-radius:4px; }}
.reportview-container .main .block-container, .main .block-container, div.block-container {{ padding-top: calc({HEADER_H}px + 18px) !important; padding-left: 1rem !important; padding-right: 1rem !important; }}
.stSidebar {{ padding-top: calc({HEADER_H}px + 18px) !important; background: {DEEP_NAVY} !important; }}
.control-header {{ color: {WHITE}; background: {DEEP_NAVY}; padding: 12px 10px; font-weight:700; border-radius:6px; text-transform: uppercase; text-align:center; font-size:18px; letter-spacing:0.6px; border: 1px solid rgba(255,255,255,0.04); }}
.stSidebar * {{ color: #ffffff !important; }}
.stSidebar input, .stSidebar select, .stSidebar textarea, .stSidebar .stButton>button, .stSidebar .stDownloadButton>button {{ background: #ffffff !important; color: #000000 !important; border-radius:6px !important; }}
.stSidebar .stCheckbox label, .stSidebar label {{ color: #ffffff !important; }}
...
.status-badge {{ background:#ffffff; color:#041723; padding:6px 10px; border-radius:16px; font-size:12px; max-width:420px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
</style>
"""
st.markdown(css, unsafe_allow_html=True)

# Render header HTML (no comments inside)
status_msg = get_app_status()
header_html = (
    '<div class="app-header" role="banner">'
    '  <div class="main-header-card" role="heading">'
    f'    <div style="width:160px; flex:0 0 160px;"><img src="{HEADER_IMAGE_B64}" alt="header"></div>'
    f'    <div style="margin-right:12px;"><div class="status-badge" title="{status_msg}">{status_msg}</div></div>'
    '    <div style="display:flex; flex-direction:column;">'
    f'      <div style="font-size:18px; color:{WHITE}; font-weight:700;">Trading - Momentum Surge Scalping</div>'
    '      <div style="font-size:11px; color:#d1e7ff; margin-top:4px;">Candles • EMA9/EMA21 • VWAP • RSI • Only BUY CE/PE</div>'
    '    </div>'
    '    <div style="margin-left:auto; width:160px;"></div>'
    '  </div>'
    '</div>'
)
