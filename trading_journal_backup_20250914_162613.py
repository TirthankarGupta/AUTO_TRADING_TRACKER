"""
trading_journal.py - UI polishing pass
Fixes applied:
1. Blend logo with header background (remove surrounding white padding)
2. Bigger 'CONTROL' font and slightly pushed up in sidebar
3. 'Actions' label text black / readable
4. 'Candle frequency' label text black / readable
Place logo.png in repo root if you want to use your PNG; otherwise an embedded SVG fallback is used.
"""
import base64
import os
import math
...
# -------------------------
# Logo embedding: prefer logo.png if present
# -------------------------
def get_embedded_logo_data_uri():
    png_path = os.path.join(os.getcwd(), "logo.png")
    if os.path.exists(png_path):
        try:
            ...
            pass
    SVG = r'''
    <svg xmlns="http://www.w3.org/2000/svg" width="1200" height="120" viewBox="0 0 1200 120" preserveAspectRatio="xMidYMid meet">
      ...
      <g transform="translate(20,14)" opacity="0.10" fill="#46c78a">
        <rect x="0" y="64" width="7" height="36" rx="1"/><rect x="22" y="60" width="7" height="40" rx="1"/>
        <rect x="44" y="56" width="7" height="44" rx="1"/><rect x="66" y="52" width="7" height="48" rx="1"/>
      </g>
      <g transform="translate(200,12) scale(0.9)" fill="#000000">
        <path d="M4 70c6-7 20-12 36-12 16 0 30 6 40 14 6 5 12 5 20 6 9 1 24 6 38 4 9-1 18-6 25-11 5-4 10-7 16-7 5 0 10 2 15 6 3 2 8 9 6 14-2 5-9 9-16 12-10 5-24 9-38 11-15 2-28 1-42-5-15-7-30-16-48-16-8 0-16 2-22 5-8 4-15 11-24 11-6 0-9-5-9-10 0-5 2-10 6-14z"/>
      </g>
      <g transform="translate(900,14) scale(0.95)" fill="#000000">
        <path d="M12 78c7-2 16-6 27-6 9 0 18 2 27 6 8 4 15 8 22 10 7 2 16 2 21-2 5-4 7-9 9-14 2-7 5-12 10-14 5-2 10 0 15 6 3 4 5 9 5 14 0 6-2 14-8 20-8 9-19 17-33 21-15 4-32 6-48 4-17-2-31-8-44-17-6-5-8-12-8-18 0-5 2-9 8-12z"/>
      </g>
    </svg>
    '''
...
LOGO_DATA_URI = get_embedded_logo_data_uri()
...
def try_get_candles():
    ...
    np.random.seed(7)
    moves = np.random.normal(loc=0.05, scale=0.7, size=periods).cumsum()
    close = base + moves
...
# -------------------------
# Streamlit config + CSS tweaks for fixes requested
# -------------------------
st.set_page_config(page_title="Trading - Momentum Surge Scalping", layout="wide", initial_sidebar_state="expanded")

css = f"""
<style>
html, body, .stApp {{ background: {CANVAS_BG}; margin:0; padding:0; }}

/* remove Streamlit padding & header so hero sits flush */
header[data-testid="stHeader"], #MainMenu, .css-1rs6os.edgvbvh3 {{
  display: none !important;
}}
/* Make the app container edges small so hero blends */
...
/* HERO: full-width feel, no white box around logo, blend logo by using transparent background */
.hero {{
  width: calc(100% - 320px);  /* leave space for sidebar area */
  margin-left: 320px;         /* position hero to the right of sidebar */
  border-radius:10px;
...
/* logo: remove any internal white padding and ensure it sits directly against the header bg */
.hero-logo {{ flex: 0 0 120px; display:flex; align-items:center; justify-content:center; }}
.hero-logo img {{ height: {LOGO_HEIGHT}px; width:auto; border-radius:6px; display:block; background:transparent; padding:0; margin:0; box-shadow:none; }}
...
/* Sidebar: deep navy; shift elements upward (smaller top padding) */
.stSidebar {{ background: {DEEP_NAVY} !important; padding-top: 12px !important; width: 320px; }}
.control-header {{ color:#ffffff; font-weight:900; font-size:20px; padding:14px; background:{DEEP_NAVY}; border-radius:8px; text-align:center; text-transform:uppercase; border:1px solid rgba(255,255,255,0.04); margin-bottom:10px; }}

/* ensure inputs/buttons readable (white backgrounds, black text) */
...
/* Make sure the placeholder / input text is clearly black */
...
/* Small catch-alls so labels for 'Actions' and 'Candle frequency' are black */
.sidebar-label-black {{ color:#000000; font-weight:600; margin-bottom:6px; display:block; }}
</style>
"""
st.markdown(css, unsafe_allow_html=True)

# -------------------------
# Render hero header (logo left, title right)
# -------------------------
hero_html = f'''
...
# -------------------------
# Sidebar controls (note: we render custom black labels for Interval and Actions)
# -------------------------
with st.sidebar:
    ...
    # Custom black label for Candle frequency to ensure readability
    ...
    # Actions label black + button below (button remains white background black text)
