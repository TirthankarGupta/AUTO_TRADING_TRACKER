"""
trading_journal.py  (Integrated Hero Header - Option B)
- Integrated hero header (non-fixed) with persistent logo left (no status pill)
- Eliminates visible gap by slight negative margin overlap (robust vs Streamlit chrome)
- Sidebar deep navy, white buttons (black text), CONTROL bigger
- Trading Journal has No. column, zebra striping
- Indicators: EMA9/EMA21, VWAP, RSI toggle
- Defensive: falls back to mock data if providers missing
Author: Assistant
Date: 2025-09-14 (Integrated Hero)
"""
...
# --------------------------
# Embedded header SVG (base64) - your chosen image (logo/trademark)
# --------------------------
SVG = r'''
<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="160" viewBox="0 0 1600 160">
  <defs><linearGradient id="softbg" x1="0" x2="0" y1="0" y2="1">
    <stop offset="0%" stop-color="#072a3a" stop-opacity="0.98"/><stop offset="100%" stop-color="#0b3546" stop-opacity="0.98"/>
  </linearGradient></defs>
  <rect width="1600" height="160" fill="url(#softbg)"/>
  <g transform="translate(80,12)" opacity="0.12" fill="#66d283">
    <rect x="0" y="90" width="8" height="40" rx="2"/><rect x="28" y="84" width="8" height="46" rx="2"/>
    <rect x="56" y="78" width="8" height="52" rx="2"/><rect x="84" y="72" width="8" height="58" rx="2"/>
    <rect x="112" y="66" width="8" height="64" rx="2"/><rect x="140" y="62" width="8" height="68" rx="2"/>
    <rect x="168" y="56" width="8" height="74" rx="2"/><rect x="196" y="50" width="8" height="80" rx="2"/>
    <rect x="224" y="44" width="8" height="86" rx="2"/><rect x="252" y="38" width="8" height="92" rx="2"/>
  </g>
...
# --------------------------
# Page config + CSS for Integrated Hero Header
# --------------------------
st.set_page_config(page_title="Trading - Momentum Surge Scalping", layout="wide", initial_sidebar_state="expanded")

# We'll place the header as the first block in content (not fixed). To visually remove the Streamlit top chrome gap
# we apply a tiny negative top-margin to the header block so it overlaps slightly and appears flush.
css = f"""
<style>
/* page background */
.stApp {{ background: {CANVAS_BG}; }}

/* remove container top padding so header sits higher */
.reportview-container .main .block-container {{ padding-top: 0rem; padding-left:1rem; padding-right:1rem; }}

/* hero header block - full width inside content */
.hero-header {{
  width: 100%;
  border-radius: 6px;
  margin-bottom: 8px;
  /* small negative top margin to visually cover tiny Streamlit chrome gap - robust across browsers */
  margin-top: -8px;
  overflow: hidden;
  box-shadow: 0 2px 8px rgba(4,10,20,0.06);
}}
...
/* logo area */
.hero-logo {{ flex: 0 0 160px; display:flex; align-items:center; justify-content:center; }}

/* textual title */
.hero-title {{ display:flex; flex-direction:column; }}
.hero-title .h1 {{ margin:0; font-size:20px; font-weight:700; color:{WHITE}; }}
.hero-title .h2 {{ margin:3px 0 0 0; font-size:11px; color:#d1e7ff; }}

/* PnL row under header */
.pnl-row {{ margin-top:10px; display:flex; gap:12px; align-items:center; }}

/* Sidebar styling: deep navy background; push down enough so controls do not overlap */
...
/* force readable inputs & buttons inside sidebar */
...
# --------------------------
# Render header (hero) - logo left, title, no status pill
# --------------------------
hero_html = f"""
<div class="hero-header">
  <div class="hero-inner">
    <div class="hero-logo"><img src="{HEADER_IMAGE_B64}" style="height:120px; width:auto; border-radius:6px;" alt="logo" /></div>
    <div class="hero-title">
      <div class="h1">Trading - Momentum Surge Scalping</div>
      <div class="h2">Candles • EMA9/EMA21 • VWAP • RSI • Only BUY CE/PE</div>
    </div>
