"""
trading_journal.py - Final replacement with Streamlit header hide + logo PNG embed if available.
Place your chosen PNG at ./logo.png to have it embedded in the header. Otherwise an SVG fallback is used.

Usage:
1. Backup current file (optional).
2. Overwrite trading_journal.py with this contents.
3. (Optional) Copy your logo PNG to project root as "logo.png".
4. Run: streamlit run trading_journal.py
"""
...
# -------------------------
# Helper: try embed logo.png if present, else use SVG fallback
# -------------------------
def get_embedded_logo_data_uri():
    ...
    # SVG fallback (crisper silhouettes). Used only if logo.png not present.
    SVG = r'''
...
# - hide Streamlit header chrome and top padding so hero sits flush
# -------------------------
st.set_page_config(page_title="Trading - Momentum Surge Scalping", layout="wide", initial_sidebar_state="expanded")

css = f"""
<style>
/* remove Streamlit top chrome / header and container top padding so header sits flush */
...
/* logo */
.hero-logo {{ flex: 0 0 180px; display:flex; align-items:center; justify-content:center; }}
.hero-logo img {{ height: {LOGO_HEIGHT}px; width:auto; border-radius:6px; }}

/* title */
.hero-title {{ display:flex; flex-direction:column; }}
.hero-title .main {{ font-size:24px; font-weight:800; line-height:1.05; }}
...
/* ensure sidebar widget text visible (catch-alls) */
.stSidebar button, .stSidebar button span, .stSidebar input, .stSidebar textarea, .stSidebar select, .stSidebar label {{
