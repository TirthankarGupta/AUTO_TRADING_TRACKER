"""
trading_journal.py  — Final single-file replacement (Polished Hero + Sidebar + Journal)
Instructions:
 1) Backup current file (you already do this).
 2) Overwrite trading_journal.py with this file.
 3) Run: streamlit run trading_journal.py
"""
import base64
import math
...
try:
    ...
    raise RuntimeError("Install streamlit: pip install streamlit") from e

try:
    import plotly.graph_objects as go
    from plotly.subplots import make_subplots
except Exception as e:
    raise RuntimeError("Install plotly: pip install plotly") from e

# -----------------------------------
# Embedded crisp SVG logo (bull + bear silhouettes)
# -----------------------------------
SVG = r'''
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="120" viewBox="0 0 1200 120" preserveAspectRatio="xMidYMid meet">
  <defs>
    <linearGradient id="g" x1="0" x2="1"><stop offset="0" stop-color="#062033"/><stop offset="1" stop-color="#071a2a"/></linearGradient>
  </defs>
  <rect width="100%" height="100%" fill="url(#g)"/>
  <!-- soft candles -->
  <g transform="translate(20,14)" opacity="0.10" fill="#46c78a">
    <rect x="0" y="64" width="7" height="36" rx="1"/>
    <rect x="22" y="60" width="7" height="40" rx="1"/>
    <rect x="44" y="56" width="7" height="44" rx="1"/>
    <rect x="66" y="52" width="7" height="48" rx="1"/>
  </g>
  <!-- Bull: left -->
...
# -----------------------------------
# Defensive: data fallbacks so demo runs without APIs
# -----------------------------------
def try_get_candles():
    try:
        import trading_data
...
# -----------------------------------
# Page config and CSS (robust)
# -----------------------------------
st.set_page_config(page_title="Trading - Momentum Surge Scalping", layout="wide", initial_sidebar_state="expanded")

# CSS: hero inside content (no fixed header), ensure sidebar padding so it does not overlap
css = f"""
<style>
html, body, .stApp {{ background: {CANVAS_BG}; margin:0; padding:0; }}
/* Make Streamlit's top container padding small so header looks integrated (robust) */
.reportview-container .main .block-container, .main .block-container, div.block-container {{
  padding-top: 0.75rem !important;
  padding-left: 1rem !important;
  padding-right: 1rem !important;
}}

/* HERO: single banner inside the page */
.hero {{
  width:100%;
  border-radius:12px;
  background: linear-gradient(90deg, #062033 0%, #071a2a 100%);
  box-shadow: 0 6px 16px rgba(7,18,28,0.06);
  color: #ffffff;
  padding: 14px 18px;
  display:flex;
  align-items:center;
...
.hero-logo img {{ height: {LOGO_HEIGHT}px; width:auto; border-radius:6px; }}
...
/* Sidebar: deep navy, readable inputs */
.stSidebar {{ background: {DEEP_NAVY} !important; padding-top: 20px !important; }}
.control-header {{ color:#ffffff; font-weight:800; font-size:18px; padding:12px; background:{DEEP_NAVY}; border-radius:6px; text-align:center; text-transform:uppercase; border:1px solid rgba(255,255,255,0.04); }}

/* ensure sidebar inputs/buttons readable */
